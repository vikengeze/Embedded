...
df = pd.DataFrame({"Seconds": [Seconds[i] for i in range(3*n)], "Υποερώτημα": [Q[i] for i in range(3*n)]})
grouped = df.groupby(["Υποερώτημα"])
...
plt.ylabel("Seconds")
plt.xlabel("Υποερώτημα")
plt.suptitle("Boxplot of all values per Question")
#plt.show()

plt.savefig("../Outputs/1.6/Boxplot_per_Q.png", dpi=400)